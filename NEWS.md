...
* `SystemRequirements` was a full prose sentence rather than a bare,
  ...
  from this repo's GitHub Actions, which already installed
  `libudev-dev` explicitly) reads this field directly to decide what
  to `apt-get install` before building -- a prose sentence very likely
